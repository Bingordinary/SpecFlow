...
4. Do not create circular dependencies between units. Check method: derive the dependency graph from all current-layer units' `unit_refs` (retiring units with `status: retired` are excluded — their references disappear with them, see `framework/unit_validate_checklist.md` retiring-unit note); a cycle (A depends on B while B depends on A, directly or transitively) is a violation. Executed by the agent during unit validate and enforced mechanically by `specflowctl validate` (Check 8 — every unit on a cycle FAILs, blocking promote) and `specflowctl deps` (reports the graph, cycle members, and promotion order). Resolution: analyze the cycle's nature — a shared contract region repeated across the cycle members' acceptance items should be extracted into a rule (making the dependencies star-shaped), or the unit boundaries should be re-drawn so the coupled contract lives in one unit. The tool reports and blocks; the user decides the resolution.
