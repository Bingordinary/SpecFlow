...
- **If any P0/P1 MISMATCH exists (verify FAIL, full run, candidate target):** delete existing `verify_result.md` if present. Do not write a failure record — a candidate full-run failure is trust establishment failed, and its delta re-run writes the record on FAIL (see §Delta re-run). Report blocking findings. Agent must stop and not proceed to promote. (A stable-only full FAIL writes a failure record with the per-item status map — see §Stable-only mode.)
...
1. **Preconditions** — the existing cache must have `mode: full` and one of the two baselines: `result: pass` (stale-cache recovery) or `result: fail` + `blocking: true` (failure-record recovery, §Failure recovery). A MISSING cache has no baseline — run `verify@{unit}` instead.
