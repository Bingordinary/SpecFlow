...
2. The CLI independently checks:
   a. Validate cache — reads `docs/specs/meta/validation/unit/{name}/validate_result.md`. If missing or stale (dependency chunk changed), rejects promote with guidance to re-run `validate`. This is the only cache gate for a retiring unit (see the Retirement note above).
