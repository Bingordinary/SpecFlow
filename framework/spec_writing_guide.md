...
Everything else in the unit (body prose, evidence appendices, background, motivation, design discussion) is not a formal behavior carrier.
...
The unit's own main spec is declared by **section regions** in validation caches: the content is split by `##` headings — the frontmatter region (file head through the line before the first `##` heading) and one region per `##` heading — and each check declares the regions its judgment read (see `framework/validation_cache.md` §Structural Region Dependencies and §Format → Per-check evidence). The region mechanism is only as precise as the section structure, so the main spec must be written to be section-splittable:
