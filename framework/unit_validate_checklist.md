...
**Dependency scope report:** Report the carrier regions actually depended on — the dependency unit's acceptance item set and the whole protocol appendix files. The item set is declared as a **structural region dependency** (`specflowctl gate-evidence --file <path> --acceptance-items`, see `framework/validation_cache.md` §Structural Region Dependencies): it is located by structure, so prose edits elsewhere in the same file — even inside the same content-defined chunk — do not stale this cache. When the no-contradiction assertion also reads a dependency unit's contract section (e.g. a protocol description in a named `##` section), declare that section region too (`--section <heading>`). Protocol appendices are contract files and are declared whole.
...
- **If any FAIL remains (after re-verification, full run, candidate target):** delete existing `validate_result.md` if present. Do not write a failure record — a candidate full-run failure is trust establishment failed, and its delta re-run writes the record on FAIL (see §Delta re-run). Proceed to Present Findings. (A stable-only full FAIL writes a failure record with the per-check status map — see §Stable-only mode.) **Why validate is the one gate that deletes on full FAIL:** validate is the upstream root — its checks judge the spec itself with no external anchor, and its holistic checks are semantically coupled in ways the declared-region mechanism cannot capture (see §Failure handling by gate role in `framework/validation_cache.md`). A `pass` judgment on an unconfirmed spec has no anchor to trust, so nothing may be carried over; the first full confirm after a FAIL must re-run everything. During the fix iteration use targeted re-checks (`validate@{unit}:check-{n}` / `:keyword`).
...
2. **Scope derivation (mechanism-derived)** — for a **pass baseline**, run `fresh@{unit}` and read its `DELTA SCOPE (validate)` section: the affected check keys (from the cache's per-check `checks` mapping), the unclaimed entries, the stale-dep count, and the degradation state. Affected checks = {checks whose declared deps went stale} ∪ {cross-check}. Stale deps that no check declared are **unclaimed** — map them by the fixed associations: a `unit:{name}` / `unit:{name}:appendix:{file}` entry → Check 7 (cross-unit); a `rule:{id}` entry → Check 8 (constraint alignment); a whole-file or declare-heavy stale dep in the unit's own file → re-read the spec and map it semantically. A cache without per-check evidence (legacy — `fresh@` reports "no per-check evidence") falls back to semantic derivation: map the changed content to the affected checks by re-reading the spec. If `fresh@{unit}` reports STALE for an entry the derivation sees as unaffected, trust `fresh@` — treat the file as a stale source and map its unclaimed deps per the rules above (see `framework/verification_scope.md` §Delta Runs). For a **failure-record baseline**, first check the record's per-check `status` map: a record **without** one (legacy) carries nothing — the re-run set is every declared check plus the cross-check, report the degradation ("the failure record carries no per-check status map — the recovery is a full re-run") and ask whether to proceed or run `validate@{unit}` instead. A record **with** the status map derives the re-run set = {checks with `status: fail`} ∪ {checks whose declared deps went stale against the current content — the fix changed content} ∪ {checks a targeted run has since found P0/P1 in — the targeted finding overrides the recorded status: the record's `pass`/`carried` evidence for that judgment was contradicted by a complete single-check judgment} ∪ {cross-check}; checks with `status` `pass`/`carried` are carried over. No stale source is required — the failed checks are the re-run reason. Report the scope — re-run checks with their source and reason, and the carried-over checks — before executing.
3. **Degradation (derived)** — when the affected set (or, for a failure record, the failed checks plus the fix's newly affected checks) covers every declared check, the incremental scope is the whole run: report this to the user ("the affected checks cover every check — an incremental re-run is a full re-run") and ask whether to proceed or run `validate@{unit}` instead. Editing one section of your own spec stales only the checks that declared it — this is NOT a degradation trigger.
4. **Execution** — re-run only the scoped checks + cross-check. Any P0/P1 finding → write a **failure record**: rewrite `validate_result.md` with `result: fail`, `blocking: true`, severity counts, findings body, `mode: full`, `basis: delta`, and the per-check `status` map (`fail`/`pass` for the re-run checks, `carried` for the carried-over ones) — the failure-recovery baseline. Stop, present findings (same as full FAIL — 5a/5h findings re-verified per §Step 9 → Check 5 extraction re-verification before presentation).
