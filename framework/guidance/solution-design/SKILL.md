...
3. Do not begin implementation from a selected option until formal truth writeback has happened — candidate spec first, then code, per `framework/concepts.md`. Gates control promotion, not the start of editing: normal units use validate/verify/review; rules and retiring units use validate only.
