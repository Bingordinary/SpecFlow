...
1. Candidate writeback is not the final review step—run `promote@{target}` as the agent-level gateway for final validation (the CLI independently validates cache freshness and format).
