...
|---------|-----------------|
| `validate@{target}` | Read-only subagent. Unit: 8-point validate checklist (`unit_validate_checklist.md`). Rule: 8-point rule metadata & body quality checklist (`rule_validate_checklist.md`). Auto-detects type from target name. Always runs all checks + cross-check — quality checks are holistic. Add `:check-{n}` or `:{keyword}` for a targeted check (targeted runs do not write a cache). Candidate target: writes the validate cache (promote gate). Stable-only target (no candidate file): confirms the stable content is still consistent with its dependencies and rules — writes `target: stable` cache; on FAIL recommends forking. See `framework/verification_scope.md`.
...
| `revalidate@{target}` | Read-only subagent. Delta: re-run only the checks whose dependency evidence went stale + cross-check, carry the rest over (unit and rule **candidate** targets). Writes a cache with `mode: full`, `basis: delta`. Stable-only targets: report "re* re-runs apply to candidate targets". See §Delta Runs in `framework/verification_scope.md`. |
