...
| `spec_verify {target}` | Read-only subagent. Unit only: 7-step spec-vs-code verify checklist (`unit_verify_checklist.md`), `:full` for all 7 steps + cross-check. If target is a Rule, report: "Rule verify has been removed. Run `spec_validate {rule}` instead." Default: scoped (git-aware). Add `:{keyword}` for specific content. On ALIGNED, conditionally triggers `spec_review` with the same scope (skips if fresh review cache exists). On MISMATCH, review is skipped. See `framework/verification_scope.md`. |
| `spec_promote {target}` | 3-step promote workflow (unit) / 2-step promote workflow (rule). Unit: candidate→stable archive (`unit_promote_workflow.md`). Rule: version promotion + consumer ref migration + body ref cleanup (`rule_promote_workflow.md`). Auto-detects type from target name. |
...
| **code review**: "review the code", "code review", "review quality" | Review code quality with spec awareness | Route to `spec_review`. Disclose review cache state if available. |
| **completion**: "it's done", "lock it in", "finalize", "promote this", "wrap it up", "ship it" | Promote the candidate to stable | Candidate exists → check validate+verify+review cache. All three fresh and non-blocking (validate PASS, verify ALIGNED, review no P0/P1) → suggest `spec_promote`. Cache missing/stale/scoped → "Pre-promote checks are not complete yet. I need to run those first. Shall I?" If review cache has P0/P1 findings, disclose: "Review found P0/P1 finding(s) — consider resolving before promoting." |
...
Review cache is optional: when the review cache has `blocking: true`, the agent must disclose P0/P1 findings before promote regardless of validate/verify state. When review cache is missing or non-blocking, the agent follows the table below as-is.
...
| Y | Y (full) | Y (full) | "Candidate has passed validate (all checks) and verify (all items)." | "If review cache exists with P0/P1 findings, disclose them and advise resolution before promote. Otherwise: 'If the design is finalized, you can promote it to stable.'" |
...
> Agent: "Candidate exists, validate has passed all checks, verify has passed all items, and review has no P0/P1 findings. Ready to promote. Running `spec_promote user_auth`..."
...
After `spec_verify` completes with ALIGNED, `spec_review` conditionally runs as a final quality gate (inheriting verify's scope). The auto-trigger checks `review_result.md` cache freshness first (see `unit_verify_checklist.md` Step 8 Procedure step 1) — if a fresh cache exists, review is skipped to avoid redundant work. The user may also trigger `spec_review` independently at any time, regardless of verify state. If verify reports MISMATCH, review is not meaningful — the code direction must first be reconciled before reviewing code quality, so it does not auto-run. `spec_review` performs a spec-aware code quality review — it inspects the code with the spec's design intent as context and suppresses findings that the spec explains.

When auto-triggered by `spec_verify`, review inherits verify's scope (scoped or full). When triggered independently, mode follows `spec_review_checklist.md` defaults. See `framework/spec_review_checklist.md` and `framework/verification_scope.md` for detail.

The review result is cached independently of the verify cache. P0/P1 findings block promote regardless of mode; only `:full` mode cache satisfies the promote gate.
