...
Caches exist only when a complete-coverage run passed (targeted runs never write caches; delta re-runs `revalidate@` / `reverify@` / `rereview@` write caches with `basis: delta`). A delta FAIL writes a **failure record** instead of deleting the cache — `result: fail` + `blocking: true` with a per-check status map, which is the failure-recovery baseline (see `framework/verification_scope.md` §Delta Runs → Failure recovery). Full-run failures delete the cache for validate/verify (candidate targets) and record a failure record for review (and stable-only targets). See `framework/verification_scope.md`.
