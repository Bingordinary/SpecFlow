...
|---------|-----------------|
| `validate@{target}` | Read-only subagent (independent single-executor session — assembly per `framework/verification_scope.md` §Sub-agent Prompt Assembly, validate shape). Unit: 8-point validate checklist (`unit_validate_checklist.md`). Rule: 8-point rule metadata & body quality checklist (`rule_validate_checklist.md`). Auto-detects type from target name. Always runs all checks + cross-check (unit targets; rules have no cross-check) — quality checks are holistic. Add `:check-{n}` or `:{keyword}` for a targeted check (targeted runs do not write a cache; targeted runs execute directly in the main agent session). Candidate target: writes the validate cache (promote gate). Stable-only target (no candidate file): confirms the stable content is still consistent with its dependencies and rules — writes `target: stable` cache; on FAIL writes a failure record and recommends forking. See `framework/verification_scope.md`.
| `verify@{target}` | Read-only subagent. Unit only: 7-step spec-vs-code verify checklist (`unit_verify_checklist.md`), always all 7 steps + cross-check. If target is a Rule, report: "Rule verify has been removed. Run `validate@{rule}` instead." Add `:{keyword}` for a targeted check (targeted runs do not write a cache). Candidate target: writes the verify cache (promote gate). Stable-only target: drift confirmation — writes `target: stable` cache; on MISMATCH writes a failure record and recommends forking. See `framework/verification_scope.md`. |
| `revalidate@{target}` | Read-only subagent (independent single-executor session — assembly per `framework/verification_scope.md` §Sub-agent Prompt Assembly, validate shape). Delta: re-run only the checks whose dependency evidence went stale + cross-check (unit targets), carry the rest over; a failure-record baseline re-runs the failed checks instead (a record without a per-check status map degrades the recovery to a full re-run — unit and rule **candidate** targets, plus stable-only targets with a usable baseline — MISSING stable caches: report "No usable confirmation baseline. Run the full command"). Writes a cache with `mode: full`, `basis: delta` (`repair` from a failure record); delta FAIL writes a failure record (`result: fail` + `blocking: true` + per-check status map). See §Delta Runs in `framework/verification_scope.md`. |
| `reverify@{unit}` | Read-only subagent. Delta: re-verify only the spec content whose evidence went stale + cross-check, carry the rest over; a failure-record baseline re-runs the failed judgments instead (candidate targets, plus stable-only targets with a usable baseline). Writes a cache with `mode: full`, `basis: delta` (`repair` from a failure record); delta FAIL writes a failure record. See §Delta Runs in `framework/verification_scope.md`. |
| `rereview@{unit}` | Read-only subagent. Delta: re-review only the files whose evidence went stale + cross-check, carry the rest over; a blocking baseline re-runs the failed dimensions instead (a blocking cache without a per-check status map degrades the recovery to a full re-run — candidate targets, plus stable-only targets with a usable baseline). Writes a cache with `mode: full`, `basis: delta` (`repair` from a blocking cache); delta FAIL writes the blocking cache extended with the per-check status map. See §Delta Runs in `framework/verification_scope.md`. |
...
Caches exist only when a complete-coverage run passed (targeted runs never write caches; delta re-runs `revalidate@` / `reverify@` / `rereview@` write caches with `basis: delta`). A delta FAIL writes a **failure record** instead of deleting the cache — `result: fail` + `blocking: true` with a per-check status map, which is the failure-recovery baseline (see `framework/verification_scope.md` §Delta Runs → Failure recovery). Full-run failures delete the cache for validate (candidate targets — the spec is the upstream root) and record a failure record for verify (candidate targets) and review (and stable-only targets). See `framework/verification_scope.md` and §Failure handling by gate role in `framework/validation_cache.md`.
...
A fresh cache always means a complete-coverage run passed — targeted runs (`:check-{n}` / `:{keyword}`) never write caches; delta runs (`revalidate@{target}` / `reverify@{unit}` / `rereview@{unit}`) write caches with `basis: delta`, and a delta FAIL writes a failure record (`result: fail` + `blocking: true`, `basis: delta`) that fresh reports as BLOCKED and that a later delta re-run recovers with `basis: repair`.
...
| `validate@{target}` (agent trigger) | Read-only subagent (independent single-executor session — assembly per `framework/verification_scope.md` §Sub-agent Prompt Assembly, validate shape). Unit: 8-point checklist (`unit_validate_checklist.md`). Rule: 8-point rule metadata & body quality checklist (`rule_validate_checklist.md`). Auto-detects type from target name. Always runs all checks + cross-check (unit targets; rules have no cross-check) — quality checks are holistic. Add `:check-{n}` or `:{keyword}` for a targeted check (targeted runs do not write a cache; targeted runs execute directly in the main agent session). See `framework/verification_scope.md`. Writes cache on PASS. On FAIL: deletes cache (candidate full run — trust establishment failed), reports findings. Agent must stop and not proceed to promote. | User says "validate" or confirms agent suggestion |
| `verify@{target}` (agent trigger) | Read-only subagent. Unit only: 7-step spec-vs-code alignment (`unit_verify_checklist.md`), always all 7 steps + cross-check. If target is a Rule, report: "Rule verify has been removed. Run `validate@{rule}` for the rule instead." Add `:{keyword}` for a targeted check (targeted runs do not write a cache). See `framework/verification_scope.md`. Writes cache on PASS. On FAIL (P0/P1 findings): writes a failure record with the per-item `status` map (the `reverify@{unit}` failure-recovery baseline — anchored to the validated spec, see §Failure handling by gate role in `framework/validation_cache.md`), reports findings, must stop, not proceed to promote. All P2/P3 → full run writes cache with `result: pass` and severity counts (non-blocking, promote allowed); targeted runs report findings without writing a cache. | User says "verify" or confirms agent suggestion |
...
| `revalidate@{target}` (agent trigger) | Read-only subagent (independent single-executor session — assembly per `framework/verification_scope.md` §Sub-agent Prompt Assembly, validate shape). Delta: re-run only the checks whose dependency evidence went stale + cross-check (unit targets), carry the rest over; a **failure record** baseline re-runs the failed checks instead (see §Delta Runs → Failure recovery). Unit and rule candidate targets, plus stable-only targets with a usable baseline (MISSING: report "No usable confirmation baseline. Run the full command" — see §Delta Runs → Layer applicability in `framework/verification_scope.md`). Writes cache with `mode: full`, `basis: delta` (`basis: repair` when recovering from a failure record). Preconditions, scope derivation, and degradation rules in §Delta Runs (`framework/verification_scope.md`). On FAIL (P0/P1): writes a **failure record** — `result: fail` + `blocking: true` + findings body + per-check status map (the failure-recovery baseline) — reports findings, must stop, not proceed to promote. | User says "revalidate" or confirms agent suggestion |
| `reverify@{unit}` (agent trigger) | Read-only subagent. Delta: re-verify only the spec content whose evidence went stale + cross-check, carry the rest over; a failure record baseline re-runs the failed judgments instead. Unit candidate targets, plus stable-only targets with a usable baseline (rule verify has been removed). Writes cache with `mode: full`, `basis: delta` (`repair` from a failure record). See §Delta Runs (`framework/verification_scope.md`). On FAIL (P0/P1): writes a failure record, reports findings, must stop, not proceed to promote. | User says "reverify" or confirms agent suggestion |
| `rereview@{unit}` (agent trigger) | Read-only subagent. Delta: re-review only the files whose evidence went stale + cross-check, carry the rest over; a blocking cache baseline re-runs the failed dimensions instead. Unit candidate targets, plus stable-only targets with a usable baseline. Writes cache with `mode: full`, `basis: delta` (`repair` from a blocking cache). See §Delta Runs (`framework/verification_scope.md`). On FAIL (P0/P1): writes cache with `blocking: true` (the failure record, same as full review FAIL, extended with the per-check status map). | User says "rereview" or confirms agent suggestion |
...
| `specflowctl gate-evidence` | Compute the dependency evidence for a file read during validate/verify/review: maps the declared line ranges onto content-defined chunks and outputs the whole-file `hash` + `deps` chunk CIDs to record in the cache. `--file <path>` required; `--ranges START-END,START-END` optional (empty = whole file); `--acceptance-items` additionally emits the structural region CID of a spec's `acceptance_item_set` (`region:acceptance_items:<cid>`) — the precise declaration for cross-unit checks; `--section <heading>` (repeatable) declares a section region by heading text (`region:section:<heading>:<cid>`) — the precise declaration for own-spec section judgments (`frontmatter` names the pre-`##` region); `--sections` lists every section region (heading, lines, CID) without declaring anything — the informational output that names `--section` values. See `framework/validation_cache.md` §Dependency Declaration and §Structural Region Dependencies. | Agent when writing a gate cache |
| `specflowctl cache-write` | Write a gate cache file with the machine-consumed evidence computed by the tooling. `--gate validate|verify|review` and `(--unit NAME | --rule ID)` required (rule targets: validate only); `--result pass|fail`; `--target candidate|stable`; `--basis full|delta|repair` (default full); `--blocking` (required on fail caches — true iff result=fail); severity counts; repeatable `--file '<json>'` declares each files entry — `{"path":...,"checks":[{"check":"1","status":"pass","sections":["Description"]}],"sections":[...],"ranges":"...","acceptance_items":bool}` (path may be a logical reference `unit:{name}` / `unit:{name}:appendix:{file}` / `rule:{id}`); the tooling computes each entry's `hash` and `deps` from the declared scope (a transcribed CID cannot enter the cache — the schema has no hash/deps fields), enforces union discipline, requires a per-check status map on failure records, then re-reads and runs the gate's own freshness chain (pass → FRESH, failure record → BLOCKED; validate@ also checks appendix coverage). Judgment (result/severity/findings/status) stays with the agent. See `framework/validation_cache.md` §Write Rules → Tooled writes. | Agent when writing a gate cache |
