...
| `revalidate@{target}` | Read-only subagent (independent single-executor session — assembly per `framework/verification_scope.md` §Sub-agent Prompt Assembly, validate shape). Delta: re-run only the checks whose dependency evidence went stale + cross-check (unit targets), carry the rest over (unit and rule **candidate** targets, plus stable-only targets with a usable confirmation baseline — MISSING/BLOCKED stable caches: report "No usable confirmation baseline. Run the full command"). Writes a cache with `mode: full`, `basis: delta`. See §Delta Runs in `framework/verification_scope.md`. |
| `reverify@{unit}` | Read-only subagent. Delta: re-verify only the spec content whose evidence went stale + cross-check, carry the rest over (candidate targets, plus stable-only targets with a usable confirmation baseline). Writes a cache with `mode: full`, `basis: delta`. See §Delta Runs in `framework/verification_scope.md`. |
| `rereview@{unit}` | Read-only subagent. Delta: re-review only the files whose evidence went stale + cross-check, carry the rest over (candidate targets, plus stable-only targets with a usable confirmation baseline). Writes a cache with `mode: full`, `basis: delta`. See §Delta Runs in `framework/verification_scope.md`. |
...
| `revalidate@{target}` (agent trigger) | Read-only subagent (independent single-executor session — assembly per `framework/verification_scope.md` §Sub-agent Prompt Assembly, validate shape). Delta: re-run only the checks whose dependency evidence went stale + cross-check (unit targets), carry the rest over. Unit and rule candidate targets, plus stable-only targets with a usable confirmation baseline (MISSING/BLOCKED: report "No usable confirmation baseline. Run the full command" — see §Delta Runs → Layer applicability in `framework/verification_scope.md`). Writes cache with `mode: full`, `basis: delta`. Preconditions, scope derivation, and degradation rules in §Delta Runs (`framework/verification_scope.md`). On FAIL (P0/P1): deletes cache, reports findings, must stop, not proceed to promote. | User says "revalidate" or confirms agent suggestion |
| `reverify@{unit}` (agent trigger) | Read-only subagent. Delta: re-verify only the spec content whose evidence went stale + cross-check, carry the rest over. Unit candidate targets, plus stable-only targets with a usable confirmation baseline (rule verify has been removed). Writes cache with `mode: full`, `basis: delta`. See §Delta Runs (`framework/verification_scope.md`). On FAIL (P0/P1): deletes cache, reports findings, must stop, not proceed to promote. | User says "reverify" or confirms agent suggestion |
| `rereview@{unit}` (agent trigger) | Read-only subagent. Delta: re-review only the files whose evidence went stale + cross-check, carry the rest over. Unit candidate targets, plus stable-only targets with a usable confirmation baseline. Writes cache with `mode: full`, `basis: delta`. See §Delta Runs (`framework/verification_scope.md`). On FAIL (P0/P1): writes cache with `blocking: true` (same as full review FAIL). | User says "rereview" or confirms agent suggestion |
