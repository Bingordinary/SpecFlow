...
3. **PASS** → write the validate cache with `target: stable` (confirmation state consumed by `fresh@stable`; same procedure as Write cache)
4. **FAIL** → write a failure record (`result: fail` + `blocking: true`, `mode: full`, `basis: full`, and the per-check `status` map — `pass`/`fail` for every executed check; rules have no cross-check and full runs have no `carried`), present the findings, and recommend forking the rule (or reconciling the consumer binding) — do not edit the stable rule directly
...
Candidate targets, plus stable-only targets with a usable baseline — a delta re-run against a stable-only target whose confirmation cache is MISSING reports "No usable confirmation baseline. Run the full command" and stops (see `framework/verification_scope.md` §Delta Runs → Layer applicability). Triggered by the user when the cache is STALE or BLOCKED (a failure record; most often: a consumer unit spec changed and the consumer-discovery checks no longer have their evidence). Follow §Delta Runs in `framework/verification_scope.md`:
...
2. **Scope derivation (mechanism-derived)** — for a **pass baseline**, run `fresh@{rule}` and read its `DELTA SCOPE (validate)` section: the affected check keys (from the cache's per-check `checks` mapping), the unclaimed entries, the stale-dep count, and the degradation state. Rule caches carry per-check evidence since the rule-checks format (see `framework/validation_cache.md` §Format → Per-check evidence): the rule file's whole-file declaration breaks down per rule-body check, and consumer-unit entries map to the checks that consumed them (Check 5/7) — a consumer-unit change stales exactly those checks. A cache without per-check evidence (legacy — written before the rule-checks format; `fresh@` reports "no per-check evidence") derives the scope semantically: a changed rule body affects the rule's own checks; a changed consumer unit spec affects the consumer-discovery checks (Check 5/7). For files whose whole-file hash changed outside the declared deps, re-scan for semantic coupling and include affected checks. If `fresh@{rule}` reports STALE for an entry the derivation sees as unaffected, trust `fresh@` — treat the file as a stale source and map it per the rules above (see `framework/verification_scope.md` §Delta Runs). For a **failure-record baseline**, first check the record's per-check `status` map: a record **without** one (legacy) carries nothing — the re-run set is every declared check (rules have no cross-check), report the degradation ("the failure record carries no per-check status map — the recovery is a full re-run") and ask whether to proceed or run `validate@{rule}` instead. A record **with** the status map derives the re-run set = {checks with `status: fail`} ∪ {checks whose declared deps went stale against the current content} ∪ {checks a targeted run has since found P0/P1 in — the targeted finding overrides the recorded status: the record's `pass`/`carried` evidence for that check was contradicted by a complete single-check judgment}; checks with `status` `pass`/`carried` are carried over. No stale source is required — the failed checks are the re-run reason. Report the scope — re-run checks with their source and reason, and the carried-over checks — before executing.
3. **Degradation** — the rule file is a contract file declared whole: any change to it stales every rule-body check, and the scope is the whole run. Report this to the user and ask whether to proceed or run `validate@{rule}` instead.
4. **Execution** — re-run only the scoped checks. Any P0/P1 finding → write a **failure record**: rewrite `validate_result.md` with `result: fail`, `blocking: true`, severity counts, findings body, `mode: full`, `basis: delta`, and the per-check `status` map (`fail`/`pass` for the re-run checks, `carried` for the carried-over ones) — the failure-recovery baseline. Stop, present findings (same as full FAIL).
5. **On PASS** — rewrite `validate_result.md` with `mode: full`, `basis: delta` (or `basis: repair` when recovering from a failure record), `target: candidate` (stable-only target: `target: stable`), a fresh `timestamp`, and a **complete** `files` list: new `hash` + `deps` + per-check `checks` evidence (`specflowctl gate-evidence`) for the re-run checks' files, the original evidence (including the per-check `checks` breakdown) for the carried-over checks' files (their CIDs are unchanged by construction — they were not stale sources). Logical references (`unit:{name}` for consumer units) are carried over the same way.
