...
2. **Scope derivation** — list the stale sources (entries whose declared dependency CIDs no longer exist) from the cache and `fresh@{rule}`; re-read the rule file and the affected unit spec files to map the changed content to the affected checks (e.g. a changed consumer unit → the consumer-discovery checks re-run; the rule's own body checks keep their evidence); for files whose whole-file hash changed outside the declared deps, re-scan for semantic coupling and include affected checks. Report the scope — re-run checks with their stale source and reason, and the carried-over checks — before executing.
3. **Degradation** — if a stale source is the candidate rule file itself, the scope is nearly the whole run: report this to the user and ask whether to proceed or run `validate@{rule}` instead.
