...
5. **On PASS** — rewrite `review_result.md` with `result: pass`, `mode: full`, `basis: delta`, `target: candidate`, `blocking: false`, severity counts, a fresh `timestamp`, a **complete** `files` list (new `hash` + `deps` + per-check `checks` evidence for re-reviewed files, original evidence for carried-over files), and the findings body.
