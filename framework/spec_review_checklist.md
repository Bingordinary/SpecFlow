...
2. Write `docs/specs/meta/validation/unit/{name}/review_result.md` per `framework/validation_cache.md` format:
   ...
   - On FAIL, include the per-dimension `status` map (`pass`/`fail` for every review dimension — the batch/file dimensions that assessed the code; full runs have no `carried`). This is the failure-recovery scope input: the recovery re-runs only the failed dimensions plus the newly stale ones (see `framework/verification_scope.md` §Delta Runs → Failure recovery). A blocking cache without the status map is an invalid write — its recovery degrades to a full re-run
...
Candidate targets, plus stable-only targets with a usable baseline — a delta re-run against a stable-only target whose confirmation cache is MISSING reports "No usable confirmation baseline. Run the full command" and stops (see `framework/verification_scope.md` §Delta Runs → Layer applicability). Triggered by the user when the cache is STALE or BLOCKED (a blocking cache — the failure record). Follow §Delta Runs in `framework/verification_scope.md`:
...
2. **Scope derivation (mechanism-derived)** — for a **pass baseline**, run `fresh@{unit}` and read its `DELTA SCOPE (review)` section: the affected check keys (from the cache's per-check `checks` mapping), the unclaimed entries, the stale-dep count, and the degradation state. Affected checks = {checks whose declared deps went stale} ∪ {cross-check}. Stale deps that no check declared are **unclaimed** — code files and dependency entries carry no per-check mapping, so map them semantically: re-read the changed file (and the spec) to identify the review dimensions that assessed it; include the cross-check; for files whose whole-file hash changed outside the declared deps, re-scan for semantic coupling and include affected files. A cache without per-check evidence (legacy — `fresh@` reports "no per-check evidence") uses the same semantic derivation for all stale sources. If `fresh@{unit}` reports STALE for an entry the derivation sees as unaffected, trust `fresh@` — treat the file as a stale source and map it per the rules above (see `framework/verification_scope.md` §Delta Runs). For a **blocking baseline**, first check the record's per-dimension `status` map: a blocking cache **without** one (legacy — written before the failure-recovery design) carries nothing — the re-run set is every declared dimension plus the cross-check, report the degradation ("the blocking cache carries no per-check status map — the recovery is a full re-run") and ask whether to proceed or run `review@{unit}` instead. A blocking cache **with** the status map derives the re-run set = {dimensions with `status: fail`} ∪ {files whose declared deps went stale against the current content — the fix changed content} ∪ {dimensions a targeted re-review has since found P0/P1 in — the targeted finding overrides the recorded status: the record's `pass`/`carried` evidence for that dimension was contradicted by a complete single-check judgment} ∪ {cross-check}; dimensions with `status` `pass`/`carried` are carried over. No stale source is required — the failed dimensions are the re-run reason. Report the scope — re-review files with their source and reason, and the carried-over files — before executing.
3. **Degradation (derived)** — when the affected set (or, for a blocking baseline, the failed dimensions plus the fix's newly affected files) covers every declared check, the incremental scope is the whole review: report this to the user ("the affected checks cover every check — an incremental re-run is a full re-run") and ask whether to proceed or run `review@{unit}` instead. An own-spec edit in a single section stales only the checks that declared that section — this is NOT a degradation trigger.
4. **Execution** — the re-run scope is executed directly by the main agent — no batching sub-agents are launched (same as the verify delta mode, see `framework/unit_verify_checklist.md` Step 7 delta-mode note). Re-review only the scoped files + cross-check. Any P0/P1 finding → write the cache with `result: fail`, `blocking: true`, severity counts, findings body, `basis: delta`, and the per-check `status` map (`fail`/`pass` for the re-run dimensions, `carried` for the carried-over ones) — the review failure record; present findings, stop.
5. **On PASS** — rewrite `review_result.md` with `result: pass`, `mode: full`, `basis: delta` (or `basis: repair` when recovering from a blocking cache), `target: candidate` (stable-only target: `target: stable`), `blocking: false`, severity counts, a fresh `timestamp`, a **complete** `files` list (new `hash` + `deps` + per-check `checks` evidence for re-reviewed files, original evidence for carried-over files), and the findings body.
