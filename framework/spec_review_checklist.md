...
Findings:
  [{severity}] {location} — {issue} (actionable | needs_decision)
    problem: ...
    evidence:
      - {verbatim quoted code} — {file:line}   # P3 findings use the fact_anchor: instead
    impact: ...
...
### Deferred findings (review only)

Findings the cross synthesis routes to another unit by recorded ownership are presented separately, after the Findings section — they are visible and retained, but they do not count toward `Key counts`, `Blocking promote`, or the gate result (see §7.5):

```
Deferred to {owner_unit}:
  [{severity}] {location} — {issue} (deferred — owned_by {owner_unit})
    problem: ...
    evidence:
      - {verbatim quoted code} — {file:line}
    impact: ...
    fix: ...   # or decision: ... with options: sub-lines
    ownership: {the recorded ownership basis} — {evidence path}
    ref: {anchor or line reference}   # optional, tracking only
```

One block per owner unit. The `ownership:` line carries the cross synthesis's record reason and its evidence path, so the report shows why the finding is not this unit's to repair. When the owner unit's own review later disposes the finding, it appears there as that unit's finding (or as a documented suppression/merge).
...
Only **gate-driving** findings decide the gate: a finding owned by this unit or unassigned. A finding deferred to another unit (a `Finding ownership:` record names that unit) is retained and routed but does not block — see §7.5 Ownership routing and `framework/verification_scope.md` §Gate Work Packets → Deferred findings.
...
4. Publish the adjusted severity in the cross result; `gate-finalize` computes the gate result from the gate-driving retained P0/P1 findings
...
### 7.5 Ownership Routing

The reviewed surface is the code this unit's spec declares, and a physical file can carry several units' behavior. Before the synthesis result is accepted, the cross executor routes every terminal retained finding to its owner:

1. For each terminal retained finding, decide whether the reviewed unit's document set records that the behavior belongs to another unit — a dependency-direction or boundary declaration in the unit's own spec (e.g. "child-run semantics are defined by the agent unit"), or another unit's spec reachable through a `--input` reference the coordinator passed to `gate-plan`. A finding whose behavior has no recorded ownership stays **unassigned** — the conservative default, and it drives this unit's gate.
2. For each finding whose recorded ownership names another unit, publish one ownership record in the cross report:
   ```
   Finding ownership: {finding_id} = owned_by {unit} — evidence: {read_ref}; reason: {one line}
   ```
   The evidence must be a read ref of the cross packet and covered by a `cross` dependency-scope declaration; the reason states the recorded ownership basis. The tool checks the record's mechanics (the read ref, the scope declaration, the unit's existence) — the executor is responsible for the judgment that the cited document really records that ownership. No record → unassigned; a deferral without recorded evidence is not available.
3. A deferred finding is retained for audit and routed — the tool excludes it from the counts, the blocking decision, and the per-key status closure, and hands it to the owner's next review through the deferred-findings ledger (`framework/verification_scope.md` §Gate Work Packets → Deferred findings). A terminal retained finding still needs its severity-confirmation sequence whether or not it is deferred.
4. **Pending deferrals from other units.** The plan loads the unit's pending ledger entries; the cross packet context lists them, and the matching file packets carry them too. The synthesis must dispose every pending deferral exactly like an input finding — `retained` (the finding becomes this unit's own; it blocks if P0/P1), `suppressed` with a reason, `merged` into a retained finding, or a new ownership record re-routing it to a third unit. A pending deferral whose file is not part of this run's review surface cannot be retained or re-routed (its key is not a logical key here) — suppress it with an evidence-backed reason; that state signals a spec-surface inconsistency (this unit's spec should declare the file its behavior lives in).

Deferral is routing, not absolution: code with no recorded design stays a finding, and without recorded ownership evidence it stays unassigned and blocks. This is what keeps spec-first intact while closing the review gate over this unit's own debt.
...
1. `gate-finalize` computes the gate result:
   - A gate-driving P0 or P1 finding exists → `result: fail`, `blocking: true`
   - Otherwise → `result: pass`, `blocking: false` (a deferred finding never blocks)
...
3. `gate-finalize` synchronizes the deferred-findings ledger: it consumes the pending deferrals this run disposed, supersedes this unit's own older deferrals for the files this run re-reviewed, and records this run's new deferrals under their owner unit (`framework/validation_cache.md` §Format → Deferred-findings ledger). The deferred findings are also recorded in the cache's `GATE_JUDGMENTS` block as `deferred_findings` for audit, and are never carried into a later run of this unit.
