...
4. **Check / Batch scope** —
   ...
   - **Delta runs (validate only):** the prompt's Check scope lists the re-run checks only; the sub-agent reports `Dependency scope` for the re-run checks it actually executed — carried-over checks are not re-executed and get no new scope declaration (their evidence stays in the cache unchanged). Delta `reverify` / `rereview` launch no sub-agent for the re-run scope — it is executed directly by the main agent (`reverify` still launches one Step 7 analysis sub-agent per mismatch per `framework/unit_verify_checklist.md`).
