...
2. **Coverage** — every required check key, acceptance item, or review file is present in the cache (validate additionally requires every non-exempt appendix). Verify analysis packets are accepted or mechanically `not_required`; cross covers every current and carried judgment and every finding, including the unit's pending deferred findings. The code surface cannot be empty by accident: verify/review planning rejects any non-`<pending>` `implementation_surface` that does not resolve to a real file (§Gate Work Packets → Input roles), so a file set can be empty only for `<pending>` items — a state verify Step 6 judges as MISMATCH
