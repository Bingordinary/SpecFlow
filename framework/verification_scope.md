...
| structural region | A content region located by structure rather than line numbers: the whole acceptance item set (`region:acceptance_items:<cid>`, from the `acceptance_item_set:` marker to the next `##` heading, or the last real line of the file), one acceptance item (`region:acceptance_item:<id>:<cid>`, from its `- id:` line to the next item), or a section region; edits outside the region do not stale a declaration on it | `framework/validation_cache.md` §Structural Region Dependencies |
