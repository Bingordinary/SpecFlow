...
A delta re-run (`revalidate@{target}` / `reverify@{unit}` / `rereview@{unit}`) that finds P0/P1 writes a **failure record** — it does not delete the cache (`result: fail` + `blocking: true`, findings body, per-check status map; `fresh` reports the gate BLOCKED). Full-run failures that write a record (candidate verify full FAIL, review full FAIL, stable-only confirmation FAIL) carry the same status map with `pass`/`fail` values only (`basis: full`). The record is the failure-recovery baseline: after the findings are resolved, the same delta re-run re-checks only the failed judgments plus the newly affected ones and carries the rest over — no full re-run needed unless the affected scope degrades to the whole run (see §Delta Runs → Failure recovery in `framework/verification_scope.md`). **A failure record without a per-check status map (legacy — written before the failure-recovery design) carries nothing: the recovery degrades to a full re-run** — a failing judgment must never be carried over as if it had passed. If the failure record itself goes stale (content changed before the recovery), derive the scope from the stale sources per the normal delta rules and re-check conservatively — the carried-over evidence of a stale record is not trusted blindly. Candidate **validate** full FAIL still deletes the cache — trust establishment failed, there is no baseline (the spec is the upstream root; see §Failure handling by gate role in `framework/validation_cache.md`); candidate verify full FAIL and review full FAIL write the blocking cache, which recovers the same way.
