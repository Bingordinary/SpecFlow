...
Unit frontmatter records identity, version, `unit_refs`, and `rule_refs` — the layer is encoded by the file path (see `framework/spec_writing_guide.md` §3). Appendix files may carry an optional `status` field (`active`, `exempt`, or `retired`) — see `framework/spec_writing_guide.md` §Appendix Files.
