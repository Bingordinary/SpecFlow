...
func writeGatePlanUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "a single path resolving to at least one real file (a directory expands")
	fmt.Fprintln(w, "recursively). Semicolon lists, wildcard patterns, nonexistent paths, and empty")
	fmt.Fprintln(w, "directories are rejected with the item id and the reason. A spec with an empty")
	fmt.Fprintln(w, "acceptance item set is rejected as well — a verify/review run has no")
	fmt.Fprintln(w, "verifiable object without at least one item.")
	fmt.Fprintln(w, "")
